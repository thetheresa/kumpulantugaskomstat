...
```
4. Let A be the event that a person tests positive for a serious virus and B be the event that the person actually has the virus. Suppose that the virus is present in 1.4% of the population. Because medical tests are sometimes incorrect, we model uncertainty by assigning probability. Suppose the conditional probability that the test is positive, given that the person has the virus, is 0.995. Also, 0.1 suppose that is the conditional probability that a person not having the virus tests positive. Determine the probability that a person actually has the virus given that the person is tested positive?
